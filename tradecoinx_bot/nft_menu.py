...
async def admin_nfts(bot, callback_query):
	buttons = [
		[
			types.InlineKeyboardButton("inBetweeners #3706", callback_data='inBetweeners3706_callback')
		],
		[
			types.InlineKeyboardButton("Cyber Bandit #4883", callback_data='CyberBandit4883_callback')
		],
		[
			types.InlineKeyboardButton("Samurai Saga #1517", callback_data='SamuraiSaga1517_callback')
		]
	]
...
async def inBetweeners3706_callback(bot, callback_query):
	buttons = [
		[
			types.InlineKeyboardButton("✅ Продать", callback_data='buy_nft_callback'),
			types.InlineKeyboardButton("↩️ Обратно", callback_data='back_nft_admin_callback')
		]
	]

	keyboard = types.InlineKeyboardMarkup(inline_keyboard=buttons)

	msg = f"💠 Токен inBetweeners 3706\n\n"\
		"🗂 Набор: <b>#1</b>\n"\
		"🔹 Блокчейн: <b>Bitcoin</b>\n\n"\
		"💸 Цена покупки: <b>28.74 USD</b> <i>(~2814,34 RUB | ~1061,29 UAH)</i>\n"\
		"💸 Цена на данный момент: <b>37.88 USD</b> <i>(~3709,37 RUB | ~1398,81 UAH)</i>"

	with open("src/coll_admin/1.png", "rb") as photo:
		await bot.send_photo(callback_query.from_user.id, photo=photo, caption=msg, reply_markup=keyboard)


async def CyberBandit4883_callback(bot, callback_query):
	buttons = [
		[
			types.InlineKeyboardButton("✅ Продать", callback_data='buy_nft_callback'),
			types.InlineKeyboardButton("↩️ Обратно", callback_data='back_nft_admin_callback')
	...
	msg = f"💠 Токен Cyber Bandit 4883\n\n"\
		"🗂 Набор: <b>#1</b>\n"\
		"🔹 Блокчейн: <b>Bitcoin</b>\n\n"\
		"💸 Цена покупки: <b>26.16 USD</b> <i>(~2561,69 RUB | ~966,02 UAH)</i>\n"\
		"💸 Цена на данный момент: <b>48.53 USD</b> <i>(~4752,26 RUB | ~1792,08 UAH)</i>"

	with open("src/coll_admin/2.png", "rb") as photo:
		await bot.send_photo(callback_query.from_user.id, photo=photo, caption=msg, reply_markup=keyboard)


async def SamuraiSaga1517_callback(bot, callback_query):
	buttons = [
		[
			types.InlineKeyboardButton("✅ Продать", callback_data='buy_nft_callback'),
			types.InlineKeyboardButton("↩️ Обратно", callback_data='back_nft_admin_callback')
	...
	msg = f"💠 Токен Samurai Saga 1517\n\n"\
		"🗂 Набор: <b>#1</b>\n"\
		"🔹 Блокчейн: <b>Bitcoin</b>\n\n"\
		"💸 Цена покупки: <b>37.53 USD</b> <i>(~3675,09 RUB | ~1385,88 UAH)</i>\n"\
		"💸 Цена на данный момент: <b>69.71 USD</b> <i>(~6826,29 RUB | ~2574,2 UAH)</i>"

	with open("src/coll_admin/3.png", "rb") as photo:
		await bot.send_photo(callback_query.from_user.id, photo=photo, caption=msg, reply_markup=keyboard)